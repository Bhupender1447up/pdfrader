...
app.post('/upload', upload.single('pdf'), async (req, res) => {
    try {
        ...
        const img = await pdfDoc.embedPng(fs.readFileSync('./image/Thesis-profiles3 (1).png'));
        ...
        const img2 = await pdfDoc.embedPng(fs.readFileSync('./image/Thesis-profiles1.png'));
        ...
        const newFilePath = path.join(__dirname, 'uploads', `${path.basename(filePath, '.pdf')}-result.pdf`);
        fs.writeFileSync(newFilePath, pdfBytes);

        // Respond with the new file details
        res.send({ message: "File uploaded and modified successfully", Filename: newFilePath });
    } catch (error) {
        console.error(error);
